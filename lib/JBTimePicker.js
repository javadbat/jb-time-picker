...
class JBTimePickerWebComponent extends HTMLElement {
    ...
    initTimeTextNodes() {
        ...
        //                                          14:41:50  
        // substitute prev
        const substitutePrevTimeHour = this.createTimeTextDOM(TimeUnits.hour, TimeSteps.substitudePrev, 18);
        const substitutePrevTimeMinute = this.createTimeTextDOM(TimeUnits.minute, TimeSteps.substitudePrev, 16);
        const substitutePrevTimeSecond = this.createTimeTextDOM(TimeUnits.second, TimeSteps.substitudePrev, 46);
        ...
        //prev
        const prevTimeHour = this.createTimeTextDOM(TimeUnits.hour, TimeSteps.prev, 19);
        const prevTimeMinute = this.createTimeTextDOM(TimeUnits.minute, TimeSteps.prev, 17);
        const prevTimeSecond = this.createTimeTextDOM(TimeUnits.second, TimeSteps.prev, 47);
        ...
        //current
        const currentTimeHour = this.createTimeTextDOM(TimeUnits.hour, TimeSteps.current, 20);
        const currentTimeMinute = this.createTimeTextDOM(TimeUnits.minute, TimeSteps.current, 18);
        const currentTimeSecond = this.createTimeTextDOM(TimeUnits.second, TimeSteps.current, 48);
        ...
        //next
        const nextTimeHour = this.createTimeTextDOM(TimeUnits.hour, TimeSteps.next, 21);
        const nextTimeMinute = this.createTimeTextDOM(TimeUnits.minute, TimeSteps.next, 19);
        const nextTimeSecond = this.createTimeTextDOM(TimeUnits.second, TimeSteps.next, 48);
        ...
        // substitute next
        const substituteNextTimeHour = this.createTimeTextDOM(TimeUnits.hour, TimeSteps.substitudeNext, 22);
        const substituteNextTimeMinute = this.createTimeTextDOM(TimeUnits.minute, TimeSteps.substitudeNext, 20);
        const substituteNextTimeSecond = this.createTimeTextDOM(TimeUnits.second, TimeSteps.substitudeNext, 50);
    ...
    handleTextDrag(e) {
        if (this.grabbedElement) {
            ...
            if (action == ActionTypes.subtrack1) {
                ...
                const prevSubstituteValue = parseInt(this.elements.prevTime[this.grabbedElement.timeUnit].innerHTML) - 1;
                ...
                this.grabbedElement.startY = e.pageY;
                // add new prev substitude element
                const nextSubstituteValue = parseInt(this.elements.nextTime[this.grabbedElement.timeUnit].innerHTML) + 1;
